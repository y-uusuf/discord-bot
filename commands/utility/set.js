...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        if (type === "view") {
            ...
            if (!config) {
                const embed = new MessageEmbed()
                    .setColor(config.embedColor).setDescription(`⚙️ <@${message.author.id}>: no settings configured for this server.`);
                return message.reply({ embeds: [embed] });
            }
        ...
        if (type === "remove") {
            ...
            if (!removeType || !validTypes[removeType]) {
                const embed = new MessageEmbed()
                    .setColor(config.embedColor).setDescription(`❌ <@${message.author.id}>: please specify which setting to remove: ${Object.keys(validTypes).join(", ")}.`);
                return message.reply({ embeds: [embed] });
            }
            ...
            const embed = new MessageEmbed()
                .setColor(config.embedColor).setDescription(`⚙️ <@${message.author.id}>: removed the **${setting.name}** setting.`);
            return message.reply({ embeds: [embed] });
        }
        ...
        if (!value && !validTypes[type]?.special) {
            const embed = new MessageEmbed()
                .setColor(config.embedColor).setDescription(`❌ <@${message.author.id}>: please provide a value.`);
            return message.reply({ embeds: [embed] });
        }
        ...
        if (setting?.special && type === "vc") {
            ...
            if (!targetTextChannel || !targetTextChannel.isText()) {
                const embed = new MessageEmbed()
                    .setColor(config.embedColor).setDescription(`❌ <@${message.author.id}>: please provide a valid **text channel** for the interface.`);
                return message.reply({ embeds: [embed] });
            }
            ...
            try {
                ...
                await targetTextChannel.send({ embeds: [embed], components: [row1, row2] });
                await message.react("✅");
            ...
            if (!role) {
                const embed = new MessageEmbed()
                    .setColor(config.embedColor).setDescription(`❌ <@${message.author.id}>: please mention a valid role or provide a role ID.`);
                return message.reply({ embeds: [embed] });
            }
            id = role.id;
            ...
            if (!channel) {
                const embed = new MessageEmbed()
                    .setColor(config.embedColor).setDescription(`❌ <@${message.author.id}>: please mention a valid channel or provide a channel ID.`);
                return message.reply({ embeds: [embed] });
            }
            id = channel.id;
        }
        ...
        const embed = new MessageEmbed()
            .setColor(config.embedColor).setDescription(`⚙️ <@${message.author.id}>: set **${setting.name}** to ${setting.isString ? `\`${id}\`` : (setting.isRole ? `<@&${id}>` : `<#${id}>`)}.`);
        return message.reply({ embeds: [embed] });
    },
};
