...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        if (!target) {
            const embed = new MessageEmbed()
                .setColor(config.embedColor).setDescription(`🔊 <@${message.author.id}>: unmutes a member.\n\n**usage:** \`,unmute @user\`\n**example:** \`,unmute @yusuf\``);
            return message.reply({ embeds: [embed] });
        }
        ...
        if (!target.roles.cache.has(roleId)) {
            const embed = new MessageEmbed()
                .setColor(config.embedColor).setDescription(`❌ <@${message.author.id}>: this user is not muted`);
            return message.reply({ embeds: [embed] });
        }
        ...
        try {
            ...
            const embed = new MessageEmbed()
                .setColor(config.embedColor).setDescription(`🔊 <@${message.author.id}>: unmuted **${target.user.username}**`);
            message.channel.send({ embeds: [embed] });

        } catch (err) {
            console.error(err);
            const embed = new MessageEmbed()
                .setColor(config.embedColor).setDescription(`❌ <@${message.author.id}>: could not unmute this user`);
