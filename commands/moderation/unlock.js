...
module.exports = {
  ...
  async execute(client, message, args) {
    ...
    if (!target) {
      const embed = new MessageEmbed()
        .setColor(config.embedColor).setDescription(`❌ <@${message.author.id}>: please provide a valid channel to unlock`);
      return message.reply({ embeds: [embed] });
    }
    ...
    try {
      ...
      const embed = new MessageEmbed()
        .setColor(config.embedColor).setDescription(`🔓 <@${message.author.id}>: unlocked <#${target.id}>`);

      message.channel.send({ embeds: [embed] });
    } catch (err) {
      console.error(err);
      const embed = new MessageEmbed()
        .setColor(config.embedColor).setDescription(`❌ <@${message.author.id}>: couldn't unlock the channel`);
